...
add_action('wp_ajax_magnitude_plugin_installer', 'magnitude_plugin_installer'); // Install plugin
add_action('wp_ajax_magnitude_plugin_activation', 'magnitude_plugin_activation'); // Activate plugin
add_action('wp_ajax_magnitude_plugin_installer_activation', 'magnitude_plugin_installer_activation'); // Activate plugin
function magnitude_plugin_installer()
{

  if (! current_user_can('install_plugins'))
    wp_die(__('Sorry, you are not allowed to install plugins on this site.', 'magnitude'));
  ...
  if (! wp_verify_nonce($nonce, 'aft_installer_nonce'))
    wp_die(__('Error - unable to verify nonce, please try again.', 'magnitude'));
...
function magnitude_plugin_activation()
{
  if (! current_user_can('install_plugins'))
    wp_die(__('Sorry, you are not allowed to activate plugins on this site.', 'magnitude'));
  ...
  // Check our nonce, if they don't match then bounce!
  if (! wp_verify_nonce($nonce, 'aft_installer_nonce'))
    die(__('Error - unable to verify nonce, please try again.', 'magnitude'));
  ...
  if ($api->name) {
    $main_plugin_file = magnitude_get_plugin_file($plugin);
    $status = 'success';
    if ($main_plugin_file) {
...
function magnitude_plugin_installer_activation()
{
  ...
  if (!empty($list_of_plugins)) {
    $data =  magnitude_activate_plugin_list($sanitinzed_activate);
  }
...
function magnitude_activate_plugin_list($sanitinzed_activate)
{
  $count = 0;
